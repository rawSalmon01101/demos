...
class altGravity3
{
    ...
    public static void main(String[] args) throws IOException, InterruptedException
    {
        ...
        //range from center of earth to sea level
        BelowSeaLevelModel(bw);

        //range from surface to earth radius from the surface of earth
        for (double i = R_EARTH + 1; i < R_EARTH * 2; i += 1.0d) //simulating dx
        {
            double g = (BIG_G * MASS_EARTH) / (i * i);
            System.out.println("Working height height: " + (i - R_EARTH) + ", g = " + g);
            bw.write(i + "," + g + "\n");
        }
    ...
    public static void BelowSeaLevelModel(BufferedWriter bw) throws IOException
    {
        for (double h = R_EARTH; h >= 0; h -= 1.0d) //simulating dx
        {
            double g_dash = SMALL_G * (1 - (h / R_EARTH));
            System.out.println("Working height: " + (R_EARTH - h) + ", g = " + g_dash);
            bw.write((R_EARTH - h) + "," + g_dash + "\n");
        }
